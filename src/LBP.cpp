...
void findSimpleLink(map<CA, CA>& LINK, map<string, map<int, int> >& SV_list_link, map<string, vector<interval> >& Linear_region ){//get deletion/duplication
	for(map<CA, CA>::iterator it = LINK.begin(); it != LINK.end(); it++){
		//cout << it->first.chr << "\t" << it->first.pos << endl;
		...
		//	cout << "pp\t" << it->first.chr << "\t" << it->first.pos << "\t" << FLAG << endl;
			if(FLAG == 0){
				...
				if(it->first.flag == "+"){//del
					...
					//assert(Linear_region[it->first.chr].size(id_2) != Linear_region[it->first.chr].end());
					//assert(Linear_region[it->first.chr].find(id_1) != Linear_region[it->first.chr].end());
...
void LoopyBeliefPropagation(map<string, vector<site> >& JOB_LIST, map<string, map<int, int> >& SV_list_CNV, map<string, map<int, int> >& SV_list_link, map<string, map<int, CA> >& SV_list, map<CA, CA>& LINK , map<site, map<hidden_state, double> >& prob_matrix_1, map<site, map<hidden_state, double> >& prob_matrix_2, map<string, vector<interval> >& Linear_region, int thread){
	...
	for(map<string, map<int, int> >::iterator it = SV_list_link.begin(); it != SV_list_link.end(); it++){
		...
		for(map<int, int>::iterator it2 = it->second.begin(); it2 != it->second.end(); it2++){
			...
			SV_anno temp(chr1, id_1_s, id_1, chr2, id_2_s, id_2, SV_list[chr1][it2->first], SV_list[chr2][LINK[SV_list[chr1][it2->first]].pos], SV_list[chr1][it2->first].mapping);
...
SV_anno::SV_anno(string chrA, int id_A_s, int id_A, string chrB, int id_B_s, int id_B, CA& s1, CA& s2, string map){
	chr1 = chrA;
	chr2 = chrB;
	id_1_s = id_A_s;
	id_1 = id_A;
...
#pragma omp parallel for num_threads(thread)
	for(int k = 0; k < CHR_LIST.size(); k++){
		...
		local_max_pre = local_max;
		local_min_pre = local_min;
		int SNP_FLAG, SNP_FLAG_pre;
		if(JOB_LIST[chr][0].type == "SNP"){
			JOB_LIST[chr][0].SNP_flag = 0;
			if(SNP_LINK[chr].find(ALL_SNP[JOB_LIST[chr][0].id - 1].pos) != SNP_LINK[chr].end())
				JOB_LIST[chr][0].SNP_flag = 1;
			if(SNP_1000G[chr].find(ALL_SNP[JOB_LIST[chr][0].id - 1].pos) != SNP_1000G[chr].end())
				JOB_LIST[chr][0].SNP_flag = 3;
		}
		for(int mi = 0; mi <= local_max/2; mi++){
			...
			for(int ma = max(local_min-mi,mi); ma <= local_max-mi; ma++){
				...
				for(int temp_i = 0; temp_i <= 1; temp_i++){
					if(temp_i == 1 && JOB_LIST[chr][0].SNP_flag == 0)
						break;
					if(temp_i == 0)
						SNP_FLAG = 0;
					else
		...
		for(i= 0; i < JOB_LIST[chr].size(); i++){
			...
			if(JOB_LIST[chr][i].type == "SNP"){
				...
				//SNP_1000G
				if(SNP_1000G[chr].find(ALL_SNP[JOB_LIST[chr][i].id - 1].pos) != SNP_1000G[chr].end())
					JOB_LIST[chr][i].SNP_flag = 3;
				if(i != 0){
					if(JOB_LIST[chr][i].id - 1-1 >= 0){
						//cout << JOB_LIST[chr][i-1].id - 1 << "\t" << JOB_LIST[chr][i].id - 1 << endl;

						if(SNP_1000G[chr].find(ALL_SNP[JOB_LIST[chr][i].id - 1 - 1].pos) != SNP_1000G[chr].end())//bug
							JOB_LIST[chr][i].SNP_flag = 4;
					}
				}
				if(SNP_LINK[chr].find(ALL_SNP[JOB_LIST[chr][i].id - 1].pos) != SNP_LINK[chr].end())
					JOB_LIST[chr][i].SNP_flag = 1;
				if(i != 0){
					if(JOB_LIST[chr][i].id - 1-1 >= 0){
						if(SNP_LINK[chr].find(ALL_SNP[JOB_LIST[chr][i].id - 1 - 1].pos) != SNP_LINK[chr].end())
							JOB_LIST[chr][i].SNP_flag = 2;
					}
				}
				if(i != 0){
					if(JOB_LIST[chr][i].id - 1-1 >= 0){
						if(SNP_1000G[chr].find(ALL_SNP[JOB_LIST[chr][i].id - 1 - 1].pos) != SNP_1000G[chr].end())
							JOB_LIST[chr][i].SNP_flag = 4;
						if(SNP_LINK[chr].find(ALL_SNP[JOB_LIST[chr][i].id - 1 - 1].pos) != SNP_LINK[chr].end())
							JOB_LIST[chr][i].SNP_flag = 2;
					}
			...
			double real_max = 1;
			int PHASE_FLAG = 0;
			// information is already considered in first iteration
			//	if(regionCov[chr][interval(JOB_LIST[chr][i].begin, JOB_LIST[chr][i].end)].flag == -1){
			...
			for(int mi = 0; mi <= local_max/2; mi++){
				...
				for(int ma = max(local_min-mi, mi); ma <= local_max-mi; ma++){
					...
					double max_p = 1;
					for(int temp_i = 0; temp_i <= 1; temp_i++){
						if(temp_i == 1 && JOB_LIST[chr][i].SNP_flag == 0)
							break;
						if(temp_i == 0)
							SNP_FLAG = 0;
						else
						...
						if(regionCov[chr][interval(JOB_LIST[chr][i].begin, JOB_LIST[chr][i].end)].flag != -1){
							if(JOB_LIST[chr][i].type == "SNP"){
								...
								if(ALL_SNP[id].sparse_flag != 1 && regionCov[chr][interval(JOB_LIST[chr][i].begin, JOB_LIST[chr][i].end)].flag != -1){ // if not sparse !
									if(SNP_FLAG == -1){
										if(local_p_freq_mi.find(ma) == local_p_freq_mi.end())
											local_p_freq_mi[ma] = norm(ma, ALL_SNP[id].minor_cov);
										if(local_p_freq_ma.find(mi) == local_p_freq_ma.end())
											local_p_freq_ma[mi] = norm(mi, ALL_SNP[id].major_cov);
										P_freq = local_p_freq_mi[ma]*local_p_freq_ma[mi];
									}
									else{
						...
						for(int mi_pre = 0; mi_pre <= local_max_pre/2; mi_pre++){
							...
							for(int ma_pre = max(local_min_pre-mi_pre, mi_pre); ma_pre <= local_max_pre-mi_pre; ma_pre++){
									...
									if(local_max_pre >= THRESHHOLD || local_max>= THRESHHOLD){
										...
										if( SV_FLAG_R.find(JOB_LIST[chr][i-1]) !=  SV_FLAG_R.end()){//LOSS
											...
											if(LO_L.find(JOB_LIST[chr][i]) !=  LO_L.end()){
												...
												//cout << "woca\t" << JOB_LIST[chr][i].begin << "\t" << SV_list_CNV[chr][JOB_LIST[chr][i].begin] << "\t" << local_del_flag_2 << endl;	
								...
								for(int temp_j = 0; temp_j <= 1; temp_j++){
									if(i == 0){
										if(temp_j == 1 && JOB_LIST[chr][0].SNP_flag == 0)
											break;
										if(temp_j == 0)
											SNP_FLAG_pre = 0;// DEFAULT
										else
											SNP_FLAG_pre = -1;

									}
									else{
										if(temp_j == 1 && JOB_LIST[chr][i-1].SNP_flag == 0)
											break;
										if(temp_j == 0)
											SNP_FLAG_pre = 0;// DEFAULT
										else
									...
									if(Prob[hidden_state(ma_pre,mi_pre,SNP_FLAG_pre,0)] < -DBL_MAX){
										continue;
									}
									if(P_cov <= 0)
									...
									if(local_max >= THRESHHOLD)
										local_p = Prob[hidden_state(ma_pre,mi_pre,SNP_FLAG_pre,0)] + log(P_cov) + log(trans) + new_pro;
									else{
										if(JOB_LIST[chr][i].type == "NOR"){
											local_p = Prob[hidden_state(ma_pre,mi_pre,SNP_FLAG_pre,0)] + log(P_cov*trans)+ new_pro;
										}
										if(JOB_LIST[chr][i].type == "SNP"){
											local_p = Prob[hidden_state(ma_pre,mi_pre,SNP_FLAG_pre,0)] + log(P_cov*P_freq*trans)+new_pro;
											if(ma_pre == ma && mi_pre == mi){
												if(JOB_LIST[chr][i].SNP_flag == 4){//IMPORTANT!!!!!!!!!!!
													int cao =  JOB_LIST[chr][i].id - 2;
													if(REF_ALT_FLAG[cao] == REF_ALT_FLAG[cao+1]){// ref always higher/lower coverage
														if(SNP_FLAG_pre == SNP_FLAG)
															local_p += log(SNP_1000G[chr][ALL_SNP[cao].pos]+0.0000000000000001);
														else
															local_p += log(1-SNP_1000G[chr][ALL_SNP[cao].pos]+0.0000000000000001);
													}
													else{
														if(SNP_FLAG_pre != SNP_FLAG)
															local_p += log(SNP_1000G[chr][ALL_SNP[cao].pos]+0.0000000000000001);
									...
									//if(51294977 <= JOB_LIST[chr][i].begin && JOB_LIST[chr][i].begin <= 51594977)
									//if(4 == JOB_LIST[chr][i].begin || JOB_LIST[chr][i].begin == 4193807 || JOB_LIST[chr][i].begin == 4194778)
									//if(i == 697 && chr == "11")
									//cout <<"sxc\t" <<  i << "\tca\t" << JOB_LIST[chr][i].begin << "\t" << ma_pre << "\t" << mi_pre << "\t" << ma << "\t" << mi << "\t" << Prob[hidden_state(ma_pre,mi_pre,SNP_FLAG_pre,0)] << "\t" << P_cov << "\t" << P_freq << "\t" << trans << "\t" << new_pro << "\t" << JOB_LIST[chr][i].if_hete << "\t" << local_p << endl;;
										//if(JOB_LIST[chr][i].type == "SNP")
											
										//cout << local_p << endl;
									if(max_p == 1 || (max_p != 1 && max_p < local_p)){
										max_p = local_p;
										temp_state = hidden_state(ma_pre,mi_pre,SNP_FLAG_pre,0);
									}
								}// snp flag end
							}//mi_pre end
							if(local_max_pre >= THRESHHOLD)
						...
						New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] = max_p;
						if(real_max == 1 || (real_max != 1 && real_max < max_p)){
							...
							if(local_del_flag_1 == 1){
								...
								//LINK[SV_list[chr1][it2->first]]
								//SV_list[chr][ LINK[ SV_list[chr][JOB_LIST[chr][i].end] ].pos ].Major = ma;
								//SV_list[chr][ LINK[ SV_list[chr][JOB_LIST[chr][i].end] ].pos ].Minor = mi;
								//cout << "wowo\t" << i << "\t" << chr << "\t" << JOB_LIST[chr][i].end << "\t" << ma << "\t" << mi << endl; 
							}
						}
						...
						if(i > 0){
							...
							New_Path[hidden_state(ma,mi,SNP_FLAG,0)].clear();
						}
				...
				for(int ma = max(mi, local_min-mi); ma <= local_max-mi; ma++){
					for(int temp_i = 0; temp_i <= 1; temp_i++){
						...
						if(temp_i == 0)
							...
							SNP_FLAG = -1;
						//if(i%50 == 0)
						//if(138217604 == JOB_LIST[chr][i].begin || JOB_LIST[chr][i].begin == 10283613 || i == 135280 || i == 135278)
						//if(236063208 <= JOB_LIST[chr][i].begin)
						//	cout << "sk\t" << i << "\t" << JOB_LIST[chr][i].begin <<"\t" << JOB_LIST[chr][i].if_hete << "\t" <<  ma << "\t" << mi << "\t" << SNP_FLAG << "\t" << New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] << endl;
						//cout << New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] - real_max << endl;
						//cout << real_max << "\t" << "sb\t" << ma << "\t" << mi << "\t" << New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] << endl;
						New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] -= real_max;
						//New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] = New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] - real_max;
						//cout << "sb\t" << ma << "\t" << mi << "\t" << New_Prob[hidden_state(ma,mi,SNP_FLAG,0)] - real_max << endl;
					}
				}
				if(local_max >= THRESHHOLD)
					break;
		...
		for(int mi = 0; mi <= local_max/2; mi++){
			...
			for(int ma = max(mi, local_min-mi); ma <= local_max-mi; ma++){
				for(int temp_i = 0; temp_i <= 1; temp_i++){
					...
					if(max_p == 1 || (max_p != 1 && max_p < Prob[hidden_state(ma,mi,SNP_FLAG,0)])){
						max_p = Prob[hidden_state(ma,mi,SNP_FLAG,0)];
						temp_state = hidden_state(ma,mi,SNP_FLAG,0);
					}
				}
			}
			if(local_max >= THRESHHOLD)
	...
	for(int i = 0; i < SV_PARALLEL_JOB.size(); i++){
		...
		if(left == right){
			if(MAX_SV != left){
				...
				SV_PARALLEL_JOB[i].conflict_flag = 2;
			}
			else{
				SV_PARALLEL_JOB[i].conflict_flag = 1;
			}
		}
		else{
			SV_PARALLEL_JOB[i].conflict_flag = 3;//incorrect
		}
...
void final_report(map<string, vector<site> >& JOB_LIST,  set<site>& SV_FLAG_L, set<site>& SV_FLAG_R, map<string, map<int, CA> >& SV_list, map<CA,CA>& LINK, vector<observe>& ALL_SNP){
					...
					if(JOB_LIST[chr][i+1].final_state.Major != Allele_A_temp && JOB_LIST[chr][i+1].final_state.Major != Allele_B_temp && JOB_LIST[chr][i+1].final_state.Minor != Allele_A_temp && JOB_LIST[chr][i+1].final_state.Minor != Allele_B_temp){
						...
						if(Allele_A_temp == Allele_B_temp){
							...
							if(Allele_A_temp == 0 || temp_CA.sv_type == "dup" && SV_PARALLEL_JOB[ CA_2_SV_anno[temp_CA] ].NUM == Allele_A_temp){
								if(SV_PARALLEL_JOB[ CA_2_SV_anno[temp_CA] ].NUM > 1){
								...
								if(temp_CA.sv_type != "dup" || (temp_CA.sv_type == "dup" && Allele_A_temp > SV_PARALLEL_JOB[ CA_2_SV_anno[temp_CA] ].NUM) ){
							...
							if(Allele_A_temp == JOB_LIST[chr][i+1].final_state.Major || Allele_A_temp == JOB_LIST[chr][i+1].final_state.Minor){// B allele changed
								CA_PHASE[temp_CA] = -1;
								if(Allele_B_temp == 0 || temp_CA.sv_type == "dup" && SV_PARALLEL_JOB[ CA_2_SV_anno[temp_CA] ].NUM == Allele_B_temp){
									...
									if(temp_CA.sv_type != "dup" || (temp_CA.sv_type == "dup" && Allele_B_temp > SV_PARALLEL_JOB[ CA_2_SV_anno[temp_CA] ].NUM) ){
										temp_CA.type = "somatic_post_aneuploid";
									}
									else{
										temp_CA.type = "somatic";
	...
	//cout << "printhe\n" << endl;
	ofstream o2("REGION_CN_PHASE");
	ofstream o1("SV_CN_PHASE");
	ofstream o3("SNP_CN_PHASE");
	for(map<string, vector <class hapBlock> >::iterator it = hapBlock_map.begin(); it != hapBlock_map.end(); it++){
		for(int i = 0; i < it->second.size(); i++){
			it->second[i].print_hapBlock(LINK, JOB_LIST, ALL_SNP, visit_sv, o2, o1, o3);
		}
	}
	...
	for(int i = 0; i < SV_PARALLEL_JOB.size(); i++){
		...
		if(local_f == 0){
			...
			o1 << SV_PARALLEL_JOB[i].SV1.chr << "\t" <<  SV_PARALLEL_JOB[i].SV1.pos << "\t" <<  SV_PARALLEL_JOB[i].SV1.flag << "\tNA\t" << SV_PARALLEL_JOB[i].SV2.chr << "\t" <<  SV_PARALLEL_JOB[i].SV2.pos << "\t" <<  SV_PARALLEL_JOB[i].SV2.flag << "\tNA\t"<< SV_PARALLEL_JOB[i].NUM << "\tNA" << "\t" << SV_PARALLEL_JOB[i].mapping << endl;
			//}
		}
	}
}
...
void hapBlock::print_hapBlock(map<CA, CA>& LINK, map<string, vector<site> >& JOB_LIST, vector<observe>& ALL_SNP, map<string, map<int, int > > & visit_sv, ofstream & o2, ofstream & o1, ofstream & o3){
	...
	for(int i = 0; i < SV_BLOCK.size(); i++){
		if(chr == LINK[SV_BLOCK[i]].chr && SV_BLOCK[i].pos < LINK[SV_BLOCK[i]].pos || chr < LINK[SV_BLOCK[i]].chr){
			...
			o1 << SV_PARALLEL_JOB[ CA_2_SV_anno[SV_BLOCK[i]] ].conflict_flag << "\t"<< SV_PARALLEL_JOB[ CA_2_SV_anno[SV_BLOCK[i]] ].mapping << "\t";
			...
			//}
			//else if(SV_BLOCK[i].type == "somatic"){
			//	o1 << LINK[SV_BLOCK[i]].type;
			//}
			//else if(){
			//	o1 <<  SV_BLOCK[i].type;
			//}
			else{
				o1 << SV_BLOCK[i].type << "/" << LINK[SV_BLOCK[i]].type;// << endl;
			}
			o1 << "\t" << SV_BLOCK[i].sv_type << endl;
		...
		for(i= 0; i < JOB_LIST[chr].size(); i++){
			...
			for(int mi = 0; mi <= local_max/2; mi++){
				...
				for(int ma = max(local_min-mi, mi); ma <= local_max-mi; ma++){
					...
					New_Path[hidden_state(ma,mi,0,0)].push_back(hidden_state(ma,mi,0,0));
				}// ma end
				if(local_max >= THRESHHOLD)
					break;
			}// mi end
			...
			for(int mi = 0; mi <= local_max/2; mi++){
				...
				for(int ma = max(mi, local_min-mi); ma <= local_max-mi; ma++){
					...
					//cout << "SSDD\t" << i << "\t" << chr << "\t" << JOB_LIST[chr][i].begin << "\t" << ma << "\t" << mi << "\t" << New_Prob[hidden_state(ma,mi,0,0)] << endl;
				}
				if(local_max >= THRESHHOLD)
					break;
			}
			...
			//cout << "cap\t" << i << "\t" << Path
			//for(map<hidden_state, vector<hidden_state> >::iterator it = Path.begin(); it!=Path.end(); it++){
			//	cout << "cap\t" << i << "\t" << JOB_LIST[chr][i].begin << "\t" << it->first.Major << "\t" << it->first.Minor << "\t" << it->second.size() << endl;
			//}
...
#pragma omp critical
		{
			...
			for(int i= 0; i < JOB_LIST[chr].size(); i++){
				int base_id = i;//i+1
				//cout << "pp\t" << base_id << endl;
				if(Path.find(temp_state) == Path.end()){
					cerr << "Path broken\n" << endl;
					cout << Path.size() << endl;
				}
				JOB_LIST[chr][i].final_state = Path[temp_state][base_id];//
				...
				//cout << "sss\t" << chr << "\t" << JOB_LIST[chr][i].begin << "\t" << JOB_LIST[chr][i].end << "\t" << JOB_LIST[chr][i].type << "\t" << Path[temp_state][base_id].Major << "\t" << Path[temp_state][base_id].Minor << "\t" << Path[temp_state][base_id].Major_base << endl;
	...
	for(int i = 0; i < SV_PARALLEL_JOB.size(); i++){
		...
		if(left == right){
			...
			SV_PARALLEL_JOB[i].conflict_flag = 3;
		}
		cout << "SV\t" << chr1 << "\t" << p1 << "\t" << chr2 << "\t" << p2 << "\t" << left << "\t" << MAX_SV << "\n";
	}
}
