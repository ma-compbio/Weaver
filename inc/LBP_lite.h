...
class SV_anno{
	public:
		SV_anno(string, int, int, string, int ,int, CA& sv1, CA& sv2);
		~SV_anno(){}
		string chr1, chr2;
...
class hapBlock{
	public:
		...
		int visited;// 0 or 1
		int phase; //+-1
		void print_hapBlock(map<CA, CA>& LINK, map<string, vector<site> >& JOB_LIST, vector<observe>& ALL_SNP, map<string, map<int,int> >& );
		hapBlock(){}
